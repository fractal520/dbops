...
def check_task():
    check_connectivity = asyncio.gather(*[asyncio.ensure_future(db_check.connectivity()) for db_check in db_check_dict.values()])

    check_connect_num = asyncio.gather(*[asyncio.ensure_future(db_check.connect_num()) for db_check in db_check_dict.values()])
